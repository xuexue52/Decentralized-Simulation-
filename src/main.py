...
from FJ.fj_metrics import calculate_fj_metrics, calculate_initial_stances
...
def main():
    ...
    initial_stances = calculate_initial_stances(len(profiles))
    ...
    for round_num in range(start_round, TOTAL_ROUNDS + 1):
        ...
        if round_num in KEY_ROUNDS:
            ...
            fj_metrics = calculate_fj_metrics(network, agents, initial_stances)
            print(f"\n=== FJ Model Metrics ===")
            print(f"NCI_thr: {fj_metrics['NCI_thr']}")
            print(f"ECI: {fj_metrics['ECI']}")
            print(f"DeltaPol: {fj_metrics['DeltaPol']}")
            print(f"GlobalDis: {fj_metrics['GlobalDis']}")
            print(f"MeanNCI: {fj_metrics['MeanNCI']}")
            
            fj_dir = os.path.join(output_dir, "FJ")
            os.makedirs(fj_dir, exist_ok=True)
            fj_metrics_file = os.path.join(fj_dir, f'fj_metrics_round_{round_num}.json')
            with open(fj_metrics_file, 'w', encoding='utf-8') as f:
                json.dump(fj_metrics, f, indent=2)
            print(f"- FJ metrics saved to '{fj_metrics_file}'")
